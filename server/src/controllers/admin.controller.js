...
exports.approveImage = async (req, res) => {
    try {
        ...
        // --- NEW: Trigger Standardized Watermarking ---
        const ffmpegService = require('../services/ffmpeg.service');
        const path = require('path');
        const fs = require('fs'); // Ensure fs is required if not globally

        // Logic to construct paths based on your file structure
        // Assuming 'imageUrl' or 'filename' stores the relative path of the raw upload
        // e.g. /uploads/raw/filename.jpg or uploads/filename.jpg

        // Let's assume image.filepath contains "/uploads/pending/filename.ext" or similar
        // Convert to absolute system path
        // Adjust this logic if your DB stores paths differently
        if (!image.filepath) throw new Error("Image record is missing 'filepath'");
        let rawFilename = path.basename(image.filepath);
        // Fallback if filepath is a full URL, we just want the filename

        // Define paths
        // Corrected: Files are in root 'uploads' folder, not 'uploads/pending'
        const inputPath = path.join(process.cwd(), 'uploads', rawFilename);
        const outputFilename = `wm_${Date.now()}_${rawFilename.split('.')[0]}.webp`;
        // Corrected: Save to 'uploads/watermarked' because app.js serves '/uploads' from '../uploads' folder
        // The previous 'public/uploads' was incorrect for the static serve config
        const outputPath = path.join(process.cwd(), 'uploads/watermarked', outputFilename);
        // UPDATED: Using 'watermark.png' as requested
        const watermarkPath = path.join(process.cwd(), 'public/watermark.png');

        console.log(`[Admin] Watermarking image ${imageId}...`);

        try {
            // Check if input file exists before processing
            if (!fs.existsSync(inputPath)) {
                console.warn(`[Admin] Input file not found at ${inputPath}, approval might fail if fallback missing...`);
            }

            // Call the NEW consistent function
            await ffmpegService.applyImageWatermark(inputPath, outputPath, watermarkPath);

            // Update image record with new watermarked URL
            // Correct key: 'watermarkedFilepath' as per Model definition
            await image.update({
                status: 'approved',
                processingStatus: 'completed',
                rejectionReason: null,
                watermarkedFilepath: `/uploads/watermarked/${outputFilename}`
            });

        } catch (wmError) {
            console.error("[Admin] Watermarking failed:", wmError);
            return res.status(500).json({
                success: false,
                message: 'Failed to process watermark during approval',
                error: wmError.message
            });
        }

        // processingStatus: 'completed' was handled in the update above
