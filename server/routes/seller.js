...
router.post('/upload-image', verifyAuth, upload.single('image'), async (req, res) => {
    try {
        ...
        const newImage = await SellerImage.create({
            ...
            thumbnailPath: thumbnailSuccess ? `/uploads/thumbnails/${thumbnailFilename}` : null,  // NEW
            originalName: req.file.originalname,
            fileSize: req.file.size,
            status: 'pending',                    // NEW: Image needs admin approval
            processingStatus: (watermarkSuccess && thumbnailSuccess) ? 'completed' : 'failed'  // NEW: Processing status
        });
